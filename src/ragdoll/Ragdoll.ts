...
export class Ragdoll extends Object3D {
  ...
  private createRagdoll() {
    ...
    createJoint(localAnchorHead, localAnchorNeck, this.head, this.torso);                             // 颈部关节
    createJoint(localAnchorRTorso, localAnchorRArm, this.torso, this.armUpperRight);                  // 右肩关节
    createJoint(localAnchorRArmBottom, localAnchorRArmLower, this.armUpperRight, this.armLowerRight); // 右肘关节
    createJoint(localAnchorLTorso, localAnchorLArm, this.torso, this.armUpperLeft);                   // 左肩关节
    createJoint(localAnchorLArmBottom, localAnchorLArmLower, this.armUpperLeft, this.armLowerLeft);   // 左肘关节
    createJoint(localAnchorLTorsoBottom, localAnchorLLegUpper, this.torso, this.thighLeft);           // 左髋关节
    createJoint(localAnchorRTorsoBottom, localAnchorRLegUpper, this.torso, this.thighRight);          // 右髋关节
    createJoint(localAnchorRLegUpperLower, localAnchorRLegLowerTop, this.thighRight, this.shinRight); // 右膝关节
    createJoint(localAnchorLLegUpperLower, localAnchorLLegLowerTop, this.thighLeft, this.shinLeft);   // 左膝关节

    // 调试时可改为 false，用于冻结刚体并观察初始关节位置。
    const enabled = true;

    for (const [key] of Object.entries(Ragdoll.boneMapping)) {
